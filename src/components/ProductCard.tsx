...
import { Star, ShoppingBag, Bookmark, Eye, Check, ShieldCheck } from 'lucide-react';
...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const staggerDelay = Math.min((index % 4) * 80, 240);

  return (
    <div
      ref={cardRef}
      style={{
        transitionDelay: isDeployed ? `${staggerDelay}ms` : '0ms',
      }}
      className={`h-full transform transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] will-change-transform ${
        isDeployed
    ...
    >
      <div
        ...
        className="group relative bg-[#111512] rounded-xl border border-[#2D302F] hover:border-[#0D9A5F] transition-all duration-300 hover:-translate-y-1.5 cursor-pointer overflow-hidden metallic-card-shadow flex flex-col justify-between h-full"
      >
        {/* Tactical Corner Brackets */}
        <div className="absolute top-1.5 left-1.5 w-2 h-2 border-t border-l border-[#2CF598]/30 group-hover:border-[#2CF598] transition-colors pointer-events-none" />
        <div className="absolute top-1.5 right-1.5 w-2 h-2 border-t border-r border-[#2CF598]/30 group-hover:border-[#2CF598] transition-colors pointer-events-none" />
        <div className="absolute bottom-1.5 left-1.5 w-2 h-2 border-b border-l border-[#2CF598]/30 group-hover:border-[#2CF598] transition-colors pointer-events-none" />
        <div className="absolute bottom-1.5 right-1.5 w-2 h-2 border-b border-r border-[#2CF598]/30 group-hover:border-[#2CF598] transition-colors pointer-events-none" />
        ...
        <div className="relative z-10 p-4 flex items-center justify-between">
          {product.badge ? (
            ...
            <span className="text-[10px] font-tech text-[#565A58] flex items-center gap-1.5 group-hover:text-[#8D918E] transition-colors">
              <span className="w-1.5 h-1.5 rounded-full bg-[#0D9A5F] group-hover:bg-[#2CF598] transition-colors" />
              <span>DEPLOYED // AR-390</span>
            </span>
          )}
